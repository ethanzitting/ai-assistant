...
- **`NUMERIC` parses to a JS number; `int8` (including `count(*)`) still arrives as a string.** postgres.js returns `NUMERIC` as a string to protect arbitrary precision, which would silently turn `total + amount` into concatenation and `amount > 100` into a lexical comparison. `db.ts` registers a `numeric` parser to fix that — verified not to affect integers or `REAL`. It does **not** cover `int8`, so keep writing `count(*)::int` (or `Number(...)`) as the existing code does.
