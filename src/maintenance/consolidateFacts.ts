...
// cluster to the consolidation model to merge into the smallest information-preserving set, then retires the
// originals (valid_until = now(), reversible) and inserts the consolidated facts.
//
// Dry-run by default. Pass --apply to review and approve each exact proposal before it is written.
...
} from "@/maintenance/proposeConsolidation.ts";
...
const show = (
  text: string,
): string => (full || text.length <= 90 ? text : `${text.slice(0, 90)}…`);
...
console.log(
  apply
    ? "── CONSOLIDATE (apply) ──"
    : "── CONSOLIDATE (dry run — pass --apply to write) ──",
);
console.log(
  `Scope: ${entityName ?? "all entities"} | edge threshold: ${threshold}${
...
console.log(
  `Found ${clusters.length} near-duplicate cluster(s); processing ${selected.length}. (today: ${today})`,
);
...
for (const [index, cluster] of selected.entries()) {
  console.log(
    `\n[${
      index + 1
    }/${selected.length}] ${cluster.entityName} — ${cluster.facts.length} facts:`,
  );
  for (const fact of cluster.facts) {
    console.log(`    - ${fact.attribute}: ${show(fact.value)}`);
  }
  ...
  inputTokens += decision.tokenUsage.inputTokens ?? 0;
  outputTokens += decision.tokenUsage.outputTokens ?? 0;

  console.log(
    `  → consolidate ${cluster.facts.length} → ${decision.consolidatedFacts.length}:`,
  );
  ...
  if (apply) {
    const approval = prompt("    Apply this exact proposal? [y/N]");
    if (!["y", "yes"].includes(approval?.trim().toLowerCase() ?? "")) {
      console.log("    skipped");
      continue;
    }
    await applyClusterConsolidation(cluster, decision);
  }
...
console.log("\n── Summary ──");
console.log(
  `${apply ? "Retired" : "Would retire"} ${plannedRetired} fact(s); ${
    apply ? "inserted" : "would insert"
  } ${plannedInserted} consolidated fact(s).`,
);
console.log(
  `Net change: ${
    plannedInserted - plannedRetired
  } live facts across ${acceptedClusters} cluster(s).`,
);
console.log(`Tokens: ${inputTokens} in / ${outputTokens} out.`);
if (!apply && plannedRetired > 0) {
  console.log("Re-run with --apply to review and approve each proposal.");
}
